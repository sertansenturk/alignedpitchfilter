...
def corect_octave_errors(pitch, notes):
    ...
    synth_pitch = notes_to_synth_pitch(notes_corrected, pitch[:, 0])

    # octave correction
    pitch_corrected = np.copy(pitch)
    for i, sp in enumerate(synth_pitch):
        pitch_corrected[i][1] = move_to_same_octave(pitch[i][1], sp)

    for nc in notes_corrected:
        trajectory = np.vstack(p[1] for p in pitch_corrected
                               if
                               nc['Interval'][0] <= p[0] <= nc['Interval'][1])
...
def notes_to_synth_pitch(notes, time_stamps, max_boundary_tol=6):
...
def find_closest_sample_idx(val, sample_vals):
    return np.argmin(abs(sample_vals - val))


def move_to_same_octave(pp, sp):
    minpp = pp
    if not (pp == 0 or sp == 0):
